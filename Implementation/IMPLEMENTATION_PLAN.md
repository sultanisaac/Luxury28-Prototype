...
- [ ] **Define Enums & Tables:** Write SQL migrations for roles, order statuses, and core tables (`users`, `products`, `orders`, `order_items`, `shipping_addresses`, etc.).
- [ ] **Implement RLS Policies:** Apply the security matrix directly to PostgreSQL using Supabase RLS.
- [ ] **Database Triggers:** 
...
- [ ] **Supabase Client Wrapper:** Setup SSR and Client-side Supabase clients using `@supabase/ssr`.
- [ ] **Authentication Flow:** Build Login, Signup/Register, and Forgot Password pages.
...
- [ ] **Manual Role Assignment:** Manually assign `admin`, `staff`, and `customer` roles to test accounts directly within the Supabase dashboard once the `users` table and triggers are active.

### Phase 4: Mockup Dashboards (Auth & Role Testing)
- [ ] **Admin Dashboard Mockup:** Create a basic `/admin` layout to verify admin access.
- [ ] **Staff Dashboard Mockup:** Create a basic `/staff` layout to verify staff access.
- [ ] **Customer Dashboard Mockup:** Create a basic `/customer` layout to verify customer access.
- [ ] **End-to-End Testing:** Verify the full flow: signup, login, password reset, and role-based redirects across all mockup dashboards.
