...
export default function UserDetailModal({ user, isOpen, onClose }: UserDetailModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl bg-zinc-950 border-zinc-800 text-white overflow-hidden p-0 gap-0">
          ...
          <div className="flex-1 p-8 overflow-y-auto space-y-8">
            <div>
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                  {orders.map((order) => (
                    <div key={order.id} className="p-4 bg-zinc-900 border border-zinc-800 rounded-xl space-y-4">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-xs font-bold text-white flex items-center gap-2">
                            Order #{order.id.slice(0, 8)}
                            <span className={`px-2 py-0.5 rounded-full text-[8px] uppercase tracking-widest border ${
                        ...
                        <p className="text-sm font-serif text-amber-500">${Number(order.total_amount).toLocaleString()}</p>
                      </div>
